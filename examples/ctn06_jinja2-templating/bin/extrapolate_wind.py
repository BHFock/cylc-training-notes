#!/usr/bin/env python3
"""
Log law wind speed extrapolation to hub height.
...
where:
    ...
    u(h)  : wind speed at hub height h [m/s]
    h     : turbine hub height [m]
    z0    : aerodynamic roughness length [m]

The roughness length z0 is passed as an environment variable set in flow.cylc
via Jinja2 templating, reflecting the site type (offshore or onshore). The hub
height is read from the turbine ancillary file.

Usage:
    python extrapolate_wind.py <forcing_file> <ancil_file> <output_file>
"""
...
def main(forcing_file, ancil_file, output_file):
    ...
    u_hub = round(u10 * math.log(hub_height / z0) / math.log(10 / z0), 4)
...
if __name__ == '__main__':
    if len(sys.argv) != 4:
        print("Usage: extrapolate_wind.py <forcing_file> <ancil_file> <output_file>")
        sys.exit(1)
